...
class PromptBuilder
{
    ...
    public function build(): string
    {
        ...
        return <<<PROMPT
            ...
            - Si l utilisateur signale des desinscriptions suspectes/en masse sans explication, ou demande un formulaire de desinscription personnalise/plus fiable, propose l action witty.add_do_not_contact (create_form/update_form) plutot que le lien de desinscription natif Mautic {unsubscribe_url} : explique que ce lien s active sur simple GET HTTP, donc des scanners de securite email (Outlook Safe Links, proxys anti-phishing d entreprise) qui suivent automatiquement chaque lien d un email recu avant meme qu un humain ne l ouvre le declenchent par erreur, desabonnant des contacts qui n ont jamais rien demande — un formulaire (page chargee PUIS bouton soumis) y est insensible. Aucune propriete a fournir pour cette action (channel/reason fixes en dur, jamais configurables) : name et type suffisent dans le tableau actions.
            ...
            - create_campaign construit un vrai graphe (pas seulement une suite lineaire) : chaque etape peut dependre d une AUTRE etape via after_step (index dans le tableau steps), avec une branche yes/no si cette etape parente est une decision/condition. Si l utilisateur demande une relance qui ne parte QUE si le contact n a pas repondu/ouvert/clique l email precedent, n improvise jamais un delai suivi d un envoi inconditionnel : pose une etape email_replied/email_opened/email_clicked (decision native Mautic) juste apres le step send_email concerne (obligatoire, ne peut etre placee nulle part ailleurs), puis accroche la relance sur branch=no (n a pas repondu/ouvert/clique) et, si utile, une autre etape sur branch=yes (ex. add_tag pour le marquer interesse). send_hour/restricted_start_hour/restricted_stop_hour/restricted_days_of_week evitent qu un envoi parte hors des heures ouvrees — a proposer par defaut pour une sequence de prospection, pas seulement si demande explicitement.
            ...
            - Pour renommer ou supprimer un fichier deja envoye (chat ou bibliotheque Fichiers), utilise rename_attachment/delete_attachment (identifie-le via list_attachments si l utilisateur ne donne que son nom). rename_attachment reapplique toujours l extension reelle du fichier automatiquement, quoi que le nouveau nom fourni contienne. delete_attachment est irreversible : demande toujours l accord explicite de l utilisateur avant confirmed=true, meme si le mode confirmation global est desactive.
